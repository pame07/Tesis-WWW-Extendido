...
var httpError_1 = require("../errorHandling/httpError");
        ...
        case 'calculateTextCredibility': {
            return [
                ...
                check('WEIGHT_TEXT_CRED_SUM_NOT_1', 'customValidation.WEIGHT_TEXT_CRED_NOT_EQUALS_TO_1')
                    .custom(function (val, obj) {
                    return Math.abs(parseFloat(obj.req.query.weightSpam) + parseFloat(obj.req.query.weightBadWords) + parseFloat(obj.req.query.weightMisspelling) - 1) < Number.EPSILON;
                }),
            ];
        }
        ...
        case 'scrapedTweetCredibility': {
            return [
                ...
                check('WEIGHT_TWEET_CRED_SUM_NOT_1', 'customValidation.WEIGHT_TWEET_CRED_NOT_EQUALS_TO_1')
                    .custom(function (val, obj) {
                    return Math.abs(parseFloat(obj.req.query.weightText) + parseFloat(obj.req.query.weightUser) + parseFloat(obj.req.query.weightSocial) - 1) < Number.EPSILON;
                }),
            ];
        }
...
function errorMapper(errors) {
    var mappedErrors = errors.map(function (error) {
        return {
            'field': error.param,
    ...
    throw new httpError_1.BadRequestError(mappedErrors);
}

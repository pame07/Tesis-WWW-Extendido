...
export function validate(method: string) : any {
  ...
  case 'calculateTextCredibility': {
    return [
      ...
      check('weightSemantic', 'weightSemantic.NOT_IN_RANGE').isFloat({min : 0, max : 100}),
      check('WEIGHT_TEXT_CRED_SUM_NOT_1', 'customValidation.WEIGHT_TEXT_CRED_NOT_EQUALS_TO_1')
        .custom((val: string, obj: any) =>
          Math.abs(parseFloat(obj.req.query.weightSpam) + parseFloat(obj.req.query.weightBadWords) + parseFloat(obj.req.query.weightMisspelling) + parseFloat(obj.req.query.weightSemantic) - 1) < Number.EPSILON),
    ]
  }
